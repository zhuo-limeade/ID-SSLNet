...
class PointPillarScatter(nn.Module):
    ...
    def forward(self, batch_dict, **kwargs):
        ...
        for batch_idx in range(batch_size):
            ...
            batch_mask = coords[:, 0] == batch_idx# 生成掩膜
            this_coords = coords[batch_mask, :]# 通过掩膜获得坐标网格
            indices = this_coords[:, 1] + this_coords[:, 2] * self.nx + this_coords[:, 3]# 
            indices = indices.type(torch.long)
            pillars = pillar_features[batch_mask, :]
            pillars = pillars.t()
            spatial_feature[:, indices] = pillars# 将采样获得的pillar安置到网格中,将不存在数据的网格置零
            batch_spatial_features.append(spatial_feature)

        batch_spatial_features = torch.stack(batch_spatial_features, 0)
        batch_spatial_features = batch_spatial_features.view(batch_size, self.num_bev_features * self.nz, self.ny, self.nx)
        batch_dict['spatial_features'] = batch_spatial_features# [B, 64, H, W]=[B, 64, 496, 432]
        return batch_dict


class PointPillarScatter3d(nn.Module):# 伪图像生成过程,进行维度分布,将(C,P)转化为(C,H,W)
    def __init__(self, model_cfg, grid_size, **kwargs):
        super().__init__()
    ...
    def forward(self, batch_dict, **kwargs):
        ...
        for batch_idx in range(batch_size):
            spatial_feature = torch.zeros(
                ...
                self.nz * self.nx * self.ny,
                dtype=pillar_features.dtype,
                device=pillar_features.device)# 生成空间网格,
