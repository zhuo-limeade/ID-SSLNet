...
class BaseBEVBackbone(nn.Module):
    ...
    def forward(self, data_dict):
        ...
        if len(self.deblocks) > len(self.blocks):
            x = self.deblocks[-1](x)
        data_dict['time4'] = time.time() - time1
        # print(x.shape)
        data_dict['spatial_features_2d'] = x
        
        return data_dict

class BaseBEVBackbone_mod0(nn.Module):
    def __init__(self, model_cfg, input_channels):
        super().__init__()
        self.model_cfg = model_cfg
...
class BaseBEVBackbone_mod1(nn.Module):
    def __init__(self, model_cfg, input_channels):
        super().__init__()
        self.model_cfg = model_cfg
...
class BaseBEVBackbone_FV(nn.Module):
    def __init__(self, model_cfg, input_channels):
        super().__init__()
        self.model_cfg = model_cfg
        ...
        self.num_bev_features = c_in
        self.blocks_fv1 = nn.Sequential(
            nn.Conv2d(
                    2880, 960, kernel_size=1,
                    stride=1, padding=0, bias=False
                ),
                nn.BatchNorm2d(960, eps=1e-3, momentum=0.01),
                nn.ReLU(),
            nn.Conv2d(
                    960, 320, kernel_size=1,
                    stride=1, padding=0, bias=False
                ),
                nn.BatchNorm2d(320, eps=1e-3, momentum=0.01),
                nn.ReLU(),

            nn.Conv2d(
                    320, 128, kernel_size=3,
                    stride=1, padding=1, bias=False
                ),
                nn.BatchNorm2d(128, eps=1e-3, momentum=0.01),
                nn.ReLU())
        self.blocks_fv2=nn.Sequential(
                nn.Conv2d(
        ...
        self.blocks_fv3 = nn.Conv2d(
                    128, 72, kernel_size=1,
                    stride=1, padding=0, bias=False
                )
    ...
    def forward(self, data_dict):
        ...
        spatial_features_fv = data_dict['spatial_features_fv']
        x = spatial_features_fv
        x = self.blocks_fv1(x)
        x = self.blocks_fv2(x)
        data_dict['spatial_features_2d_fv_extra'] = x
        x = self.blocks_fv3(x)
        data_dict['spatial_features_2d_fv'] = x
        

        # print('spatial_features_2d_fv', x.shape)
...
class BaseBEVBackbone_mod1(nn.Module):
    def __init__(self, model_cfg, input_channels):
        ...
        self.num_bev_features = c_in
        self.ssfa = SSFA(layer_nums=[5,],ds_layer_strides=[1,], ds_num_filters=[128,], us_layer_strides=[1,], us_num_filters=[128,],
