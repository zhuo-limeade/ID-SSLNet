...
class VoxelResBackBone8xVoxelNeXt(nn.Module):
    ...
    def bev_out(self, x_conv):
        features_cat = x_conv.features# [N1+N2+N3, 128]
        indices_cat = x_conv.indices[:, [0, 2, 3]]# 已转为BEV,仅需要batchsize和h,w
        spatial_shape = x_conv.spatial_shape[1:]# [180*180]

        indices_unique, _inv = torch.unique(indices_cat, dim=0, return_inverse=True)# 消除重复的indice
        features_unique = features_cat.new_zeros((indices_unique.shape[0], features_cat.shape[1]))# 用于存储去除重复索引后的唯一特征
        features_unique.index_add_(0, _inv, features_cat)# 原始特征按照索引映射关系添加到新特征中
    ...
    def forward(self, batch_dict):
        ...
        x_conv4 = self.conv4(x_conv3)# [B, 180, 180]
...
class VoxelResBackBone8xVoxelNeXt_mod1(nn.Module):
    def __init__(self, model_cfg, input_channels, grid_size, **kwargs):
        super().__init__()
        self.model_cfg = model_cfg
        norm_fn = partial(nn.BatchNorm1d, eps=1e-3, momentum=0.01)
        ...
        self.mapping_conv = spconv.SparseSequential(
            spconv.SubMConv2d(out_channel, 10, 3, stride=1, padding=1, bias=True),
        )
    ...
    def forward(self, batch_dict):
        ...
        x_conv5.indices[:, 1:] *= 2
        x_conv6.indices[:, 1:] *= 4
        # x_conv4 = x_conv4.replace_feature(x_conv6.features)
        # x_conv4.indices = x_conv6.indices
        x_conv4 = x_conv4.replace_feature(torch.cat([x_conv4.features, x_conv5.features, x_conv6.features]))
        x_conv4.indices = torch.cat([x_conv4.indices, x_conv5.indices, x_conv6.indices])
        out = self.bev_out(x_conv4)
        ...
        cls_map = self.mapping_conv(out)# 得到[cls,180,180]空间特征
...
class VoxelResBackBone8xVoxelNeXt_distill(nn.Module):
    def __init__(self, model_cfg, input_channels, grid_size, **kwargs):
        super().__init__()
        self.model_cfg = model_cfg
        norm_fn = partial(nn.BatchNorm1d, eps=1e-3, momentum=0.01)
