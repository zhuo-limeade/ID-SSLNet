...
def gaussian_radius(height, width, min_overlap=0.5):
    ...
    r1 = (b1 + sq1) / 2# 第一种情况,两角点在真值框内

    a2 = 4
    b2 = 2 * (height + width)
    c2 = (1 - min_overlap) * width * height
    sq2 = (b2 ** 2 - 4 * a2 * c2).sqrt()
    r2 = (b2 + sq2) / 2# 第二种情况,两角点在真值框外

    a3 = 4 * min_overlap
    b3 = -2 * min_overlap * (height + width)
    c3 = (min_overlap - 1) * width * height
    sq3 = (b3 ** 2 - 4 * a3 * c3).sqrt()
    r3 = (b3 + sq3) / 2# 第三种情况,一角点在真值框外,一角点在真值框内
    ret = torch.min(torch.min(r1, r2), r3)# 得到三高斯半径的最小值
    return ret
...
def draw_gaussian_to_heatmap_voxels(heatmap, distances, radius, k=1):
    diameter = 2 * radius + 1
    sigma = diameter / 6
    masked_gaussian = torch.exp(- distances / (2 * sigma * sigma))# 二维距离高斯分布，distance为欧式距离平方

    torch.max(heatmap, masked_gaussian, out=heatmap)# 将高斯分布安置到hm中
...
def _topk_1d(scores, batch_size, batch_idx, obj, K=40, nuscenes=False):
    ...
    for bs_idx in range(batch_size):
        ...
        topk_classes = (topk_ind // K).int()# 通过该步获得前k个得分分别对应head中的哪个类别(会生成重复部分,如何解决??)
...
def decode_bbox_from_voxels_nuscenes(batch_size, indices, obj, rot_cos, rot_sin,
    ...
    scores, inds, class_ids = _topk_1d(None, batch_size, batch_idx, obj, K=K, nuscenes=True)# obj为热图，对热图求socre前k=500个有效体素indice
    ...
    ret_pred_dicts = []
    for k in range(batch_size):# 求heatmap大于阈值的项目
        cur_mask = mask[k]
        cur_boxes = final_box_preds[k, cur_mask]
        cur_scores = final_scores[k, cur_mask]
        cur_labels = final_class_ids[k, cur_mask]
