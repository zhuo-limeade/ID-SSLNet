...
class VoxelRCNNHead(RoIHeadTemplate):
    ...
    def roi_grid_pool(self, batch_dict):
        ...
        for k, src_name in enumerate(self.pool_cfg.FEATURES_SOURCE):
            ...
            for bs_idx in range(batch_size):
                cur_voxel_xyz_batch_cnt[bs_idx] = (cur_coords[:, 0] == bs_idx).sum()# 每个batch有效voxel数目
    ...
    def get_dense_grid_points(rois, batch_size_rcnn, grid_size):
        ...
        dense_idx = faked_features.nonzero()  # (N, 3) [x_idx, y_idx, z_idx],返回张量中每个非零元素的索引,快速生成坐标
        dense_idx = dense_idx.repeat(batch_size_rcnn, 1, 1).float()  # (B, 6x6x6, 3)

        local_roi_size = rois.view(batch_size_rcnn, -1)[:, 3:6]
        roi_grid_points = (dense_idx + 0.5) / grid_size * local_roi_size.unsqueeze(dim=1) \
                          - (local_roi_size.unsqueeze(dim=1) / 2)  # (B, 6x6x6, 3), (dense_idx + 0.5) / grid_size将网格坐标标准化并偏移到网格中心,后面的乘法与减法是将其偏移到roi中心
