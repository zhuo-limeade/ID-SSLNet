...
class ProposalTargetLayer(nn.Module):
    ...
    def subsample_rois(self, max_overlaps):
        ...
        fg_thresh = min(self.roi_sampler_cfg.REG_FG_THRESH, self.roi_sampler_cfg.CLS_FG_THRESH)# 得到前景的阈值

        fg_inds = ((max_overlaps >= fg_thresh)).nonzero().view(-1)# 得到前景点索引
        ...
        hard_bg_inds = ((max_overlaps < self.roi_sampler_cfg.REG_FG_THRESH) &
                (max_overlaps >= self.roi_sampler_cfg.CLS_BG_THRESH_LO)).nonzero().view(-1)# 分等级划分索引

        fg_num_rois = fg_inds.numel()# 得到前景索引的数量
        bg_num_rois = hard_bg_inds.numel() + easy_bg_inds.numel()# 得到背景索引的数量
