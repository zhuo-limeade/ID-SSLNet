...
class Voxel_Wise_Mapping_Hard(nn.Module):
    ...
    def forward(self, sp_tensor):
        ...
        sp_tensor_indice = sp_tensor.indices
        ...
        for n in range(self.num_split):# 按照clus_indice将将原有的体素特征拆分为num_cluster个新体素特征
            ...
            temp_indice = sp_tensor_indice[map_indice==n]
            if temp_indice == None:# 问题是某个batch上可能没有值而不是特征上没有值，TODO：对不存在值的batch添加一个值或者对后续检测头部分进行改动，不计算该部分损失
                mapping_features.append(sp_tensor)
                print('yes!')
            else:
...
class Voxel_Wise_Mapping_Soft(nn.Module):
    def __init__(self, in_channels, num_split):
        super().__init__()
        self.num_split = num_split
        self.mapping_conv = spconv.SubMConv2d(in_channels, self.num_split, 1, stride=1, padding=0, bias=True)#
    ...
    def forward(self, sp_tensor):
        ...
        # _, map_indice = torch.max(mapping_sp_tensor.features, dim=1)# 得到每个点通道特征对应的最大值的feature与indice
        map_indice = F.sigmoid(mapping_sp_tensor.features)
        map_indice = map_indice>0.5
        
        mapping_features = []
        for n in range(self.num_split):# 按照clus_indice将将原有的体素特征拆分为num_cluster个新体素特征
            # 设计了硬式概率热图拆分,考虑设计软式拆分(直接归一化加权)
            temp_feature = sp_tensor_feature[map_indice[:,n]]
            # sp_tensor_feature*map_indice[:,n]+sp_tensor_feature
            temp_indice = sp_tensor_indice[map_indice[:,n]]
            sparse_tensor = spconv.SparseConvTensor(# 将特征重组为张量
                features = temp_feature,
                indices = temp_indice,
...
class VoxelNeXtHead_MappingSplit(nn.Module):
    def __init__(self, model_cfg, input_channels, num_class, class_names, grid_size, point_cloud_range, voxel_size,
        ...
        self.build_losses()
        self.num_head = len(self.class_names_each_head)
        self.voxel_mapping = Voxel_Wise_Mapping_Soft(in_channels=input_channels, num_split=self.num_head)
        self.class_project = {}
    ...
    def assign_targets(self, gt_boxes, num_voxels, spatial_indices, spatial_shape):
        ...
        for idx, cur_class_names in enumerate(self.class_names_each_head):
            ...
            for bs_idx in range(batch_size):
                ...
                if spatial_indices[idx][bs_idx].shape[0] == 0:# 20240625 modified by ken.
                    spatial_indices[idx][bs_idx] = torch.zeros([1,2], device=spatial_indices[idx][bs_idx].device)
                    print('yes')
                heatmap, ret_boxes, inds, mask = self.assign_target_of_single_head(
                    ...
                    num_voxels=num_voxels[idx][bs_idx], spatial_indices=spatial_indices[idx][bs_idx], 
                    spatial_shape=spatial_shape[idx],
    ...
    def assign_target_of_single_head(
        ...
        for k in range(min(num_max_objs, gt_boxes.shape[0])):# 一般为gt的数目
            if dx[k] <= 0 or dy[k] <= 0:
                continue
            ...
            distance = self.distance(spatial_indices, center[k])# 求出BEV空间位置与每个gt中心的距离
            inds[k] = distance.argmin()# 筛选与gt位置最近的体素
            # try:
            #     inds[k] = distance.argmin()# 筛选与gt位置最近的体素
            # except:
            #     print([self.forward_ret_dict['mapping_features'][i].features.shape for i in range(6)])
            #     print([self.forward_ret_dict['num_voxels']])
            #     exit()
            mask[k] = 1
            # 将半径内
    ...
    def get_loss(self):
        ...
        tb_dict = {}
        loss = 0
        batch_indices, spatial_indices = [], []
        for n in range(self.num_head):# 拆分体素特征的batchsize与坐标
            batch_indices.append(self.forward_ret_dict['voxel_indices'][n][:, 0])
            spatial_indices.append(self.forward_ret_dict['voxel_indices'][n][:, 1:])

        for idx, pred_dict in enumerate(pred_dicts):# 数目为检测头数
            pred_dict['hm'] = self.sigmoid(pred_dict['hm'])# 将heatmap进行sigmoid处理
            hm_loss = self.hm_loss_func(pred_dict['hm'], target_dicts['heatmaps'][idx])# 前景热图损失计算
            hm_loss *= self.model_cfg.LOSS_CONFIG.LOSS_WEIGHTS['cls_weight']

            target_boxes = target_dicts['target_boxes'][idx]
            pred_boxes = torch.cat([pred_dict[head_name] for head_name in self.separate_head_cfg.HEAD_ORDER], dim=1)# head_order为每一类的预测项目,hm等

            reg_loss = self.reg_loss_func(
                pred_boxes, target_dicts['masks'][idx], target_dicts['inds'][idx], target_boxes, batch_index[idx]
            )
            ...
            if self.iou_branch:
                batch_box_preds = self._get_predicted_boxes(pred_dict, spatial_indices[idx])
                ...
                iou_loss = self.crit_iou(pred_dict['iou'], target_dicts['masks'][idx], target_dicts['inds'][idx],
                                            pred_boxes_for_iou, target_dicts['gt_boxes'][idx], batch_indices[idx])

                iou_reg_loss = self.crit_iou_reg(batch_box_preds, target_dicts['masks'][idx], target_dicts['inds'][idx],
                                                    target_dicts['gt_boxes'][idx], batch_indices[idx])
    ...
    def _get_voxel_infos(self, x):
        ...
        for bs_idx in range(batch_size):# 将sptensor形式的张量按照batch进行拆分
            ...
            if voxel_indices[batch_inds].shape[0] == 0:
                print('_get_voxel_infos', batch_inds.sum())
        return spatial_shape, batch_index, voxel_indices, spatial_indices, num_voxels

    def forward(self, data_dict):
        x = data_dict['encoded_spconv_tensor']

        mapping_features = self.voxel_mapping(x)

        spatial_shape, batch_index, voxel_indices, spatial_indices, num_voxels = [], [], [], [], []
        for n in range(self.num_head):
            temp_spatial_shape, temp_batch_index, temp_voxel_indices, temp_spatial_indices, temp_num_voxels = self._get_voxel_infos(mapping_features[n])
            spatial_shape.append(temp_spatial_shape)
            batch_index.append(temp_batch_index)
            voxel_indices.append(temp_voxel_indices)
            spatial_indices.append(temp_spatial_indices)
            num_voxels.append(temp_num_voxels)
        self.forward_ret_dict['batch_index'] = batch_index
        

        pred_dicts = []
        for index, head in enumerate(self.heads_list):
            pred_dicts.append(head(mapping_features[index]))
        ...
        self.forward_ret_dict['mapping_features'] = mapping_features
