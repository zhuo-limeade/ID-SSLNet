...
class Class_Irrelevant_Head_DEP1(nn.Module):
    ...
    def build_HSM(self, in_channels):
        self.HSM_avg_pool = nn.AdaptiveAvgPool2d(1)  # 输出形状 [B, C, 1, 1]
        self.HSM_max_pool = nn.AdaptiveMaxPool2d(1)

        # 两层全连接网络：先降维再升维
        self.HSM_fc = nn.Sequential(
    ...
    def HSM_forward(self, x:dict):
        curr_hctable = []
        for x_idx in range(len(x)):
            if x_idx == len(x)-1:   continue
            avg_out = self.fc(self.avg_pool(x[x_idx]))
            max_out = self.fc(self.max_pool(x[x_idx]))
            out = avg_out + max_out
            attention = self.sigmoid(out)
            _, indice = torch.topk(attention, 2, dim=1)
            self.curr_hctable.append(indice)
        curr_hctable.append(torch.ones(indice.shape))
