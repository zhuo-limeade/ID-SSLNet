...
def process_single_sequence_and_save(sequence_file: str, save_path: Path, has_label:bool=True,
    ...
    txt_path = os.path.join(cur_save_dir, 'info.txt')
    os.makedirs(os.path.join(cur_save_dir), exist_ok=True)
    
    file = open(txt_path, 'w')
    w = file.write('time of day: ' + sequence_infos[0]['time_of_day']) 
    file.close()
    
    print(sequence_infos[0]['time_of_day'])
