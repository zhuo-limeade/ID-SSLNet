...
class SSL_Homemade_Dataset(DatasetTemplate):
    ...
    def get_lidar(self, idx):
        lidar_file = self.root_split_path / 'ssl' / ('%s.bin' % idx)
    ...
    def get_label(self, idx):
        ...
        with open(label_file, 'r') as f:
            ...
            for line in lines:
                ...
                labels['dimensions'].append(np.array(line[8:11], dtype=np.float32))
                labels['location'].append(np.array(line[11:14], dtype=np.float32))
    ...
    def evaluation(self, det_annos, class_names, **kwargs):
        ...
        from .kitti_object_eval_python import eval_ssl as eval_ssl

        eval_det_annos = copy.deepcopy(det_annos)
        eval_gt_annos = [copy.deepcopy(info['annos']) for info in self.kitti_infos]
        PR_detail_dict = {}
        ap_result_str, ap_dict = eval_ssl.get_official_eval_result(eval_gt_annos, eval_det_annos, class_names, PR_detail_dict)
...
def create_kitti_infos(dataset_cfg, class_names, data_path, save_path, workers=4):
    dataset = SSL_Homemade_Dataset(dataset_cfg=dataset_cfg, class_names=class_names, root_path=data_path, training=False)
...
def create_kitti_infos_dis(dataset_cfg, class_names, data_path, save_path, workers=4):
    dataset = SSL_Homemade_Dataset(dataset_cfg=dataset_cfg, class_names=class_names, root_path=data_path, training=False)
    val_split = 'train'

    val_filename = save_path / ('ssl_infos_5m_%s.pkl' % val_split)
...
if __name__ == '__main__':
    ...
    dataset_cfg = EasyDict(yaml.safe_load(open('/home/ken/workspace/OpenPCDet/tools/cfgs/dataset_configs/ssl_homemade_dataset.yaml')))
    ROOT_DIR = '/home/ken/workspace/OpenPCDet/data/ssl_dataset_full'
