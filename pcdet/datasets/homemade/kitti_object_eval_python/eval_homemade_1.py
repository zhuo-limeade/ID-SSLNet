...
def clean_data(gt_anno, dt_anno, current_class, difficulty):
    CLASS_NAMES = ['little_car', 'Cone','Pedestrian']
    MIN_HEIGHT = [-.1, -.1, -.1]
    MAX_OCCLUSION = [0, 1, 2]
...
def eval_class(gt_annos,
    ...
    # f = open('/home/ken/program/dataset/result_process/KITTI_VIZ_3D/val_dict/1.txt', 'w+')
    # f.write(str(ret_dict['recall'][:,:,1,:]))
    # for n in range(3):
    #     for n in range 
    # f.write('\n')
    # # f.write(str(ret_dict['precision'][:,:,1,:]))
    # f.close()
    return ret_dict
...
def do_eval(gt_annos,
    ...
    #     # 筛选gt
    #     # gt_mask = gt_anno["location"][:, 0] < 8 # x方向<5
    #     # gt_mask = (gt_anno["location"][:, 0] >= 5)& (gt_anno["location"][:, 0] < 10) # x方向<5
    #     gt_mask = gt_anno["location"][:, 0] >= 8
    #     filtered_gt = {}
    #     for key in gt_anno:
    #         if isinstance(gt_anno[key], np.ndarray) and len(gt_anno[key]) == len(gt_mask):
    #             filtered_gt[key] = gt_anno[key][gt_mask]
    #         else:
    #             filtered_gt[key] = gt_anno[key]
    #     # print(filtered_gt['location'])
    #     if not filtered_gt['location'].any():
    #         # print("a")
    #         continue
    #     filtered_gt_annos.append(filtered_gt)
        
    #     # 筛选dt
    #     # dt_mask = dt_anno["location"][:, 0] < 8  # x方向<5
    #     # dt_mask = (dt_anno["location"][:, 0] >= 5)& (dt_anno["location"][:, 0] < 10) # x方向<5
    #     dt_mask = dt_anno["location"][:, 0] >= 8
    #     filtered_dt = {}
    #     for key in dt_anno:
    #         if isinstance(dt_anno[key], np.ndarray) and len(dt_anno[key]) == len(dt_mask):
    #             filtered_dt[key] = dt_anno[key][dt_mask]
    #         else:
    #             filtered_dt[key] = dt_anno[key]
    #     filtered_dt_annos.append(filtered_dt)
    # dt_annos = filtered_dt_annos
    # gt_annos = filtered_gt_annos
...
def get_official_eval_result(gt_annos, dt_annos, current_classes, PR_detail_dict=None):
    ...
    overlap_0_5 = np.array([[0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5],
                                                        ...
                                                        [0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5]])
    # overlap_0_5 = np.array([[0.3, 0.3, 0.3, 0.3, 00.3, 0.3, 0.3],
    #                                                     [0.3, 0.3, 0.3, 0.3, 00.3, 0.3, 0.3],
    #                                                     [0.3, 0.3, 0.3, 0.3, 00.3, 0.3, 0.3],])
    min_overlaps = np.stack([overlap_0_5], axis=0)  # [1,1,7]
    class_to_name = {
        0:'little_car',
        1:'Cone',
        2:'Pedestrian',
    ...
    for j, curcls in enumerate(current_classes):
        ...
        mean+=mAP3d_R40[j, 0, i]
    mean /=7
    result += print_str((f"MEAN 3d   AP:{mean:.4f}, "
                                 f"{mean:.4f}, "
                                 f"{mean:.4f}"))
