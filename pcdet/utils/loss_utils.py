...
def neg_loss_cornernet(pred, gt, mask=None):
    ...
    Returns:
    """
    pos_inds = gt.eq(1).float()# 按位等于
    neg_inds = gt.lt(1).float()# 按位小于
...
class RegLossSparse(nn.Module):
    ...
    # output为pred_boxes,即为hm
    # mask为指示ind(0-500)是否被填充的标志向量,
    #ind为输出的每个体素特征图中最大的体素,
    def forward(self, output, mask, ind=None, target=None, batch_index=None):
        ...
        for bs_idx in range(batch_size):
            ...
            if output[batch_inds].shape[0] == 0:
                temp_tensor = torch.zeros([1,10], device=output.device)
                pred.append(temp_tensor[ind[bs_idx]])# 对与目标匹配的体素进行筛选,
                print('RegLossSparse')
                continue
                
            pred.append(output[batch_inds][ind[bs_idx]])# 对与目标匹配的体素进行筛选,
