...
def boxes_iou_normal(boxes_a, boxes_b):
    ...
    # 得到两个bbox的外接矩形左上角和右下角的值
    x_min = torch.max(boxes_a[:, 0, None], boxes_b[None, :, 0])# [70400,4],[14,4],none仅作占位
...
def boxes3d_lidar_to_aligned_bev_boxes(boxes3d):
    ...
    """
    rot_angle = common_utils.limit_period(boxes3d[:, 6], offset=0.5, period=np.pi).abs()# 得到gtbox的rot
    # 布置x,y的box,将中心点转换为边界框
    choose_dims = torch.where(rot_angle[:, None] < np.pi / 4, boxes3d[:, [3, 4]], boxes3d[:, [4, 3]])# 按照rot选择box的朝向            
    aligned_bev_boxes = torch.cat((boxes3d[:, 0:2] - choose_dims / 2, boxes3d[:, 0:2] + choose_dims / 2), dim=1)# size=[box_num,4]
...
def boxes3d_lidar_to_aligned_bev_boxes_fv_box(boxes3d):
    """
    Args:
        boxes3d: (N, 7 + C) [x, y, z, dx, dy, dz, heading] in lidar coordinate
    ...
    # 布置x,y的box,将中心点转换为边界框
    choose_dims = torch.where(rot_angle[:, None] < np.pi / 4, boxes3d[:, [5, 3]], boxes3d[:, [5, 4]])
    aligned_bev_boxes = torch.cat((boxes3d[:, :3][:, ::2] - choose_dims / 2, boxes3d[:, :3][:, ::2] + choose_dims / 2), dim=1)
    return aligned_bev_boxes

def boxes3d_lidar_to_aligned_bev_boxes_fv_anchor(boxes3d):
    """
    Args:
        boxes3d: (N, 7 + C) [x, y, z, dx, dy, dz, heading] in lidar coordinate
    ...
    rot_angle = common_utils.limit_period(boxes3d[:, 6], offset=0.5, period=np.pi).abs()# 得到gtbox的rot
    # 布置x,y的box,将中心点转换为边界框
    choose_dims = torch.where(rot_angle[:, None] < np.pi / 4, boxes3d[:, [5, 3]], boxes3d[:, [5, 4]])# 按照rot选择box的朝向            
    aligned_bev_boxes = torch.cat((boxes3d[:, 0:2] - choose_dims / 2, boxes3d[:, 0:2] + choose_dims / 2), dim=1)# size=[box_num,4]
    return aligned_bev_boxes



def boxes3d_nearest_bev_iou_fv(boxes_a, boxes_b):
    """
    Args:
        boxes_a: (N, 7) [x, y, z, dx, dy, dz, heading]
        boxes_b: (N, 7) [x, y, z, dx, dy, dz, heading]
    ...
    """
    boxes_bev_a = boxes3d_lidar_to_aligned_bev_boxes_fv_anchor(boxes_a)
    boxes_bev_b = boxes3d_lidar_to_aligned_bev_boxes_fv_box(boxes_b)
