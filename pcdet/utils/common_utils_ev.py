...
def merge_results_dist_annos(result_part, size, tmpdir):
    ...
    for res in zip(*part_list):
        ordered_results.extend(sum(list(res), []))
...
class IndexDict():
    ...
    def get(self, id):
        if id in self.id_to_num:
            ...
            self.number_id += 1
            self.id_to_num[id] = self.number_id
            self.num_to_id[self.number_id] = id
