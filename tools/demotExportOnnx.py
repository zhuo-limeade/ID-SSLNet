...
datapath = 'OpenPCDet/tools/n015-2018-07-24-11-22-45+0800__LIDAR_TOP__1532402927647951.pcd.bin'
cfg_ifle = 'OpenPCDet/tools/cfgs/nuscenes_models/cbgs_voxel0075_voxelnext.yaml'
ckpt = 'OpenPCDet/tools/voxelnext_nuscenes_kernel1.pth'
datapath = os.path.join(abPath, datapath)
cfg_ifle = os.path.join(abPath, cfg_ifle)
ckpt = os.path.join(abPath, ckpt)
onnx_path = os.path.join(abPath, 'OpenPCDet/tempTest/outPutONNX/voxelnext.onnx')
...
class DemoDataset(DatasetTemplate):
    ...
    def __getitem__(self, index):
        if self.ext == '.bin':
            points = np.fromfile(self.sample_file_list[index], dtype=np.float32).reshape(-1, 8)[:, 0:5]
        elif self.ext == '.npy':
...
def main():
    ...
    with torch.no_grad():
        ...
        for idx, data_dict in enumerate(demo_dataset):
            ...
            outputNames = ['center', 'center_z', 'dim', 'rot', 'vel', 'hm']
            # onnx 导出
            torch.onnx.export(model, data_dict, onnx_path, input_names = ['data2VoxelFeature'], output_names = outputNames)
